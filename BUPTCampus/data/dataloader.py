...
from tools.sampler import RandomIdentitySampler, ConsistentModalitySampler, RandomCameraSampler
...
def get_dataloader(opt, mode, show=False):
    ...
    if mode == 'train':
        if opt.train_sampler is None:
            ...
            # the final batch size is equal to `Np * Nc * Nt * 2
    ...
    def __getitem__(self, index):
        ...
        if self.mode in ('train', 'auxiliary'):
            ...
            for idx in frame_idx:
                idx = int(idx)
                img_path = data_paths_ir[idx]
                img_path_ir = join(self.data_root, img_path)
                img = read_image(img_path_ir)

                img = np.array(img)
                if self.transform is not None:
                    img = self.transform(img)

                images_ir.append(img)
            images_ir = torch.cat(images_ir, dim=0)

            images_rgb = []
            for idx in frame_idx:
                idx = int(idx)
                img_path = data_paths_rgb[idx]
                img_path_rgb = join(self.data_root, img_path)
                img = read_image(img_path_rgb)
            ...
            images_rgb = torch.cat(images_rgb, dim=0)
            # images_ir = torch.cat(
            #     [self.transform(
            ...
            return images_rgb, label, CAMERA[camera], images_ir, label, CAMERA[camera]
            ...
            for idx in frame_idx:
                ...
                img_path = join(self.data_root, img_path)
