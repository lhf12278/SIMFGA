...
def train(epoch, wG):
    ...
    for batch_idx, (imgs_ir, pids_ir, camid_ir, imgs_rgb, pids_rgb, camid_rgb) in enumerate(trainloader):
        ...
        labels_list = []
        head_labels = Variable(torch.ones(loader_batch*2).long().cuda())
        body_labels = Variable(torch.zeros(loader_batch*2).long().cuda())
        ...
        feat, out0, logits_list, en_feature, en_feature_p, feat1, feat1_p, de_feature, de_feature_p, spatial_feat, spatial_feat_p, logits_list1= net(input1, input2, seq_len=seq_lenth)
        ...
        loss_kl = criterion3(F.softmax(de_feature_p1 / args.T, dim=1), F.softmax(feat1_p.detach() / args.T, dim=1)) + criterion3(F.softmax(de_feature_p2 / args.T, dim=1), F.softmax(feat1_p.detach() / args.T, dim=1))
